...
def get_db_handle():
    ...
    try:
        ...
        if mongodb_uri is None or '<' in mongodb_uri:
            logger.warning("MongoDB URI not properly configured. Set MONGODB_URI in your .env — no credentials are hardcoded.")
            mongodb_uri = os.environ.get('MONGODB_URI', 'mongodb+srv://<username>:<password>@<cluster>.mongodb.net/IntelliDB?retryWrites=true&w=majority')
